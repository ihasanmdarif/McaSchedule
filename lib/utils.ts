...
export const dateToString = (passedDate: string) => {
...
export const formattedAppDate = (passedDate: string) => {
  const date = new Date(`${passedDate}T00:00:00`);
  const month = date
    .toLocaleDateString("en-CA", { month: "short" })
    .toUpperCase();

  // Get weekday in uppercase (e.g., MON)
  const weekday = date
    .toLocaleDateString("en-CA", { weekday: "short" })
    .toUpperCase();

  // Get day of the month (e.g., 5)
